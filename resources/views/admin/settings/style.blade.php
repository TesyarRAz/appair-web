<form action="{{ route('admin.settings.update', 'style') }}" method="POST" class="card shadow" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="card-body" x-data="{ type: '{{ $style->bg_type }}'}">
        ...
        <div class="form-group">
            <nav class="nav nav-pills nav-justified mb-2">
				...
				<a class="nav-item nav-link" href="#" x-on:click="type = 'image'" :class="type == 'image' && ['active', 'bg-info']">Gambar</a>
				<a class="nav-item nav-link" href="#" x-on:click="type = 'color'" :class="type == 'color' && ['active', 'bg-info']">Warna</a>
			</nav>
        </div>
		<div class="form-group" x-if="type == 'color'">
            <label>Warna Background</label>
            <input name="bg_color" type="color" class="form-control" min="0" value="{{ $style->bg_color }}" required>
        </div>
        <div class="form-group" x-if="type == 'image'">
