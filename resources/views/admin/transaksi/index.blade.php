...
<div class="card shadow">
    ...
    <div class="card-body">
		<form class="form-group" action="{{ route('admin.transaksi.index') }}" method="get" id="form-filter-status">
    		<nav class="nav nav-pills nav-justified mb-2" x-data="{ status: '{{ in_array(request()->status, ['diterima', 'ditolak', 'lunas']) ? request()->status : 'diterima' }}' }" x-init="$watch('status', () => $('#form-filter-status').trigger('change'))">
